/*
  ...
  - A unique constraint covering the columns `[accountId]` on the table `agents` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `accountId` to the `agents` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `agents` ADD COLUMN `accountId` VARCHAR(100) NOT NULL,
...
-- CreateIndex
CREATE UNIQUE INDEX `agents_accountId_key` ON `agents`(`accountId`);

-- AddForeignKey
ALTER TABLE `agents` ADD CONSTRAINT `agents_accountId_fkey` FOREIGN KEY (`accountId`) REFERENCES `accounts`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
